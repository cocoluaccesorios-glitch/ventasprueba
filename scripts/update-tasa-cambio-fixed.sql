...
-- VERSIÓN CORREGIDA CON PRECISIÓN ADECUADA
-- =====================================================

-- Paso 1: Agregar columna fecha_hora
...
-- Paso 4: Función para obtener la tasa más reciente del día (CORREGIDA)
CREATE OR REPLACE FUNCTION get_latest_tasa_bcv_today()
RETURNS DECIMAL(15,8) AS $$
DECLARE
    latest_tasa DECIMAL(15,8);
BEGIN
...
-- Paso 5: Función para verificar si existe una tasa diferente hoy (CORREGIDA)
CREATE OR REPLACE FUNCTION check_tasa_different_today(new_tasa DECIMAL(15,8))
RETURNS BOOLEAN AS $$
DECLARE
    latest_tasa DECIMAL(15,8);
BEGIN
...
-- Paso 6: Función principal para verificar y actualizar tasa automáticamente (CORREGIDA)
CREATE OR REPLACE FUNCTION verificar_y_actualizar_tasa_bcv(new_tasa DECIMAL(15,8))
RETURNS JSON AS $$
DECLARE
    resultado JSON;
    tasa_existente DECIMAL(15,8);
    diferencia DECIMAL(15,8);
    tolerancia DECIMAL(15,8) := 0.01;
...
-- Paso 8: Verificar que la tabla se actualizó correctamente
SELECT 
...
-- Paso 9: Probar la función principal
