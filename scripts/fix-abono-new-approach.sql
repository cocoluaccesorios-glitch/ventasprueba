-- =====================================================
-- NUEVO ENFOQUE: CREAR COLUMNA NUEVA SIN TOCAR LA EXISTENTE
-- =====================================================
...
-- PASO 3: COPIAR DATOS A LA NUEVA COLUMNA
...
SELECT 
    ...
    p.total_abono_usd * COALESCE(p.tasa_bcv_nueva, 166.5800) as monto_abono_ves,
    COALESCE(p.tasa_bcv_nueva, 166.5800) as tasa_bcv,
...
SELECT DISTINCT
    ...
    COALESCE(p.tasa_bcv_nueva, 166.5800) as tasa_bcv
FROM pedidos p
WHERE p.tasa_bcv_nueva IS NOT NULL
AND p.tasa_bcv_nueva > 0
-- Evitar duplicados
AND NOT EXISTS (
...
SELECT 
    ...
    p.total_usd,
    COALESCE(p.tasa_bcv_nueva, 166.5800) as tasa_bcv,
    p.total_usd * COALESCE(p.tasa_bcv_nueva, 166.5800) as total_ves,
    COALESCE(SUM(a.monto_abono_usd), 0) as total_abonado_usd,
    COALESCE(SUM(a.monto_abono_usd), 0) * COALESCE(p.tasa_bcv_nueva, 166.5800) as total_abonado_ves,
    calcular_saldo_pendiente(p.id) as saldo_pendiente_usd,
    calcular_saldo_pendiente(p.id) * COALESCE(p.tasa_bcv_nueva, 166.5800) as saldo_pendiente_ves,
    p.fecha_pedido,
    p.estado_entrega,
...
GROUP BY p.id, p.cliente_cedula, p.cliente_nombre, p.cliente_apellido, 
         p.total_usd, p.tasa_bcv_nueva, p.fecha_pedido, p.estado_entrega;
