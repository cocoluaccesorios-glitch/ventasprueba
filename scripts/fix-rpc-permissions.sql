-- =====================================================
-- CORRECCIÓN DE PERMISOS PARA FUNCIONES RPC
-- =====================================================
...
GRANT EXECUTE ON FUNCTION procesar_venta_completa(JSONB) TO authenticated;
GRANT EXECUTE ON FUNCTION procesar_venta_simple(JSONB) TO authenticated;
GRANT EXECUTE ON FUNCTION procesar_venta_mixta(JSONB) TO authenticated;
...
SELECT 
    'Permisos otorgados a authenticated' as seccion,
    'Funciones RPC y tablas ahora tienen permisos correctos' as resultado;

-- PASO 7: CREAR POLÍTICA RLS PARA PERMITIR INSERCIONES
-- Crear política RLS para permitir inserciones en pedidos
CREATE POLICY IF NOT EXISTS "Allow authenticated users to insert pedidos" 
ON pedidos FOR INSERT 
TO authenticated 
WITH CHECK (true);

-- Crear política RLS para permitir inserciones en detalles_pedido
CREATE POLICY IF NOT EXISTS "Allow authenticated users to insert detalles_pedido" 
ON detalles_pedido FOR INSERT 
TO authenticated 
WITH CHECK (true);

-- Crear política RLS para permitir inserciones en abonos
CREATE POLICY IF NOT EXISTS "Allow authenticated users to insert abonos" 
ON abonos FOR INSERT 
TO authenticated 
WITH CHECK (true);

-- Crear política RLS para permitir inserciones en tasa_cambio
CREATE POLICY IF NOT EXISTS "Allow authenticated users to insert tasa_cambio" 
...
-- PASO 10: PROBAR LA FUNCIÓN CON DATOS DE PRUEBA
SELECT 
