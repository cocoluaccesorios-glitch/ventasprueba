-- =====================================================
-- CORRECCIÓN SEGURA DE PRECISIÓN NUMÉRICA
-- =====================================================
...
-- PASO 2: CAMBIAR TIPOS DE DATOS A NUMERIC SIN PRECISIÓN (MÁXIMA FLEXIBILIDAD)
-- Esto evita completamente el overflow numérico

-- Cambiar columnas de pedidos
ALTER TABLE pedidos ALTER COLUMN total_usd TYPE numeric;
ALTER TABLE pedidos ALTER COLUMN total_abono_usd TYPE numeric;
ALTER TABLE pedidos ALTER COLUMN monto_abono_usd TYPE numeric;
ALTER TABLE pedidos ALTER COLUMN monto_abono_ves TYPE numeric;
ALTER TABLE pedidos ALTER COLUMN monto_abono_simple TYPE numeric;

-- Cambiar columnas de abonos
ALTER TABLE abonos ALTER COLUMN monto_abono_usd TYPE numeric;
ALTER TABLE abonos ALTER COLUMN monto_abono_ves TYPE numeric;
ALTER TABLE abonos ALTER COLUMN tasa_bcv TYPE numeric;

-- Cambiar columnas de tasa_cambio
ALTER TABLE tasa_cambio ALTER COLUMN tasa_bcv TYPE numeric;

-- PASO 3: CREAR TABLA TASA_CAMBIO SI NO EXISTE
...
-- PASO 4: CREAR TABLA ABONOS SI NO EXISTE
CREATE TABLE IF NOT EXISTS abonos (
    ...
    monto_abono_usd numeric NOT NULL DEFAULT 0,
    monto_abono_ves numeric NOT NULL DEFAULT 0,
    tasa_bcv numeric NOT NULL,
...
-- PASO 5: AGREGAR COLUMNAS FALTANTES A LA TABLA PEDIDOS
...
ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS monto_abono_simple numeric DEFAULT 0;
ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS monto_abono_usd numeric DEFAULT 0;
ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS monto_abono_ves numeric DEFAULT 0;
ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS total_abono_usd numeric DEFAULT 0;
...
-- PASO 6: ACTUALIZAR PEDIDOS EXISTENTES DE ABONO
UPDATE pedidos 
SET 
    es_abono = TRUE,
...
-- PASO 7: MIGRAR ABONOS EXISTENTES A LA TABLA ABONOS
INSERT INTO abonos (
    pedido_id,
    cliente_id,
    monto_abono_usd,
...
-- PASO 8: MIGRAR TASAS BCV EXISTENTES
INSERT INTO tasa_cambio (fecha, tasa_bcv)
SELECT DISTINCT
...
-- PASO 9: CREAR FUNCIÓN PARA CALCULAR SALDO PENDIENTE
...
-- PASO 10: CREAR VISTA PARA RESUMEN DE DEUDAS
...
-- PASO 11: VERIFICAR CREACIÓN Y MIGRACIÓN
SELECT 
...
-- PASO 12: MOSTRAR PEDIDOS DE ABONO ACTUALIZADOS
SELECT 
...
-- PASO 13: MOSTRAR ABONOS MIGRADOS
SELECT 
...
-- PASO 14: VERIFICAR SALDOS PENDIENTES
SELECT 
